						 ...
						 
{
    ...
    if( zone[0].daylength_flag >0){
        zone[0].metv.dayl *= 0.5;// to average NOVAA dayl and topographic effect;
        if(zone[0].metv.dayl > 86400.0 || zone[0].metv.dayl <= 14400.0){
            printf("zone_dailyF (%d,%d,%d,%d)-(%e)\n",
                   zone[0].ID, current_date.year, current_date.month, current_date.day,
                   zone[0].metv.dayl);// greater 24 hr or less than 4 hours a day
        }//if
    }//if
	if ( (zone[0].daylength_flag == 0) && (zone[0].e_horizon == 0) && (zone[0].w_horizon == 0) ){
        // daylength_flag=0 means no "observed" daylength data from the inputs
        
        // only apply to "flat" area with no input data. otherise, it's calculated in the zone_hourly
        zone[0].metv.dayl = basin[0].daylength;
		if (zone[0].Kdown_direct_flag == 0){
            // Kdown_direct_flag=0 means no "observed" data from the inputs
			zone[0].Kdown_direct = zone[0].Kdown_direct * ( basin[0].daylength / zone[0].metv.dayl );
		}
		if (zone[0].Kdown_diffuse_flag == 0){
            // Kdown_direct_flag=0 means no "observed" data from the inputs
			zone[0].Kdown_diffuse = zone[0].Kdown_diffuse * ( basin[0].daylength / zone[0].metv.dayl );
		}
	}//if flat land
	...
	if (zone[0].snow == -999.0 ){
        //tavg; tmin
		if (zone[0].metv.tmin < zone[0].defaults[0][0].max_snow_temp ){
	...
	if ( (zone[0].Kdown_direct_flag == 0) ||
        ...
        * zone[0].Kdown_direct_adjustment;//<--- calculated in zone_daily_I; default value is ONE if cloud fraction is not observed.
...
//                e_dewpoint_tmp = compute_saturation_vapor_pressure(tmin_tmpn);
...
//            }else{
//                zone[0].METV_vpd_21ravgNext60[jj] = -1.0;
//            }
//        }//jj
        
	}// given VPD is not provided
	...
	/*--------------------------------------------------------------*/
	zone[0].metv.swavgfd = (zone[0].Kdown_direct + zone[0].Kdown_diffuse) / zone[0].metv.dayl * 1000.0  ; //zone[0].metv.dayl can be zero?
	/*--------------------------------------------------------------*/
	/*	compute total ppfd		umol photon/m2*s	*/
	/*	umol photon/m2*s = u mol photon/m2*day * (1 day/dayl s)	*/
	/*--------------------------------------------------------------*/
	zone[0].metv.ppfd = (zone[0].PAR_direct + zone[0].PAR_diffuse) / zone[0].metv.dayl ;
